...
1. **Sessão client-side defasada pós-login (causa da falha do TC013 e do provável falso-positivo do TC001):** o `SessionProvider` do NextAuth não refetcha após o redirect soft do server action do `/login`; em headless `useSession()` fica `undefined` e `ServiceItem` cai no ramo não autenticado (hoje: navega para `/login`). Correção sugerida: navegação full-page (`window.location`) após o login de teste e tratar `status === "loading"` em `handleBookingClick` em vez de `data?.user`.
2. ~~**Diálogo de login só oferece Google:** o `SignInDialog` não tem link para `/login` — beco sem saída para credenciais~~ — **RESOLVIDO em 25/09:** os pontos de entrada (ícone do cabeçalho, gaveta mobile e "Reservar") agora navegam para `/login`, que virou a página única de login: formulário de teste (sob `TEST_LOGIN_ENABLED`) + botão **Entrar com Google**, reaproveitando o `SignInDialog` fora de diálogo. O Google OAuth voltou a ser alcançável pela interface.
