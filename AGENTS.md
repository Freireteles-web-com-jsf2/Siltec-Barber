...
- SQLite is the only configured provider. Use a `file:` URL such as `file:./prisma/dev.db`; runtime code must use the lazy `db`/`getPrisma` wrapper, with `prisma/seed.ts` as the only direct-client exception. Keep native adapter packages external and deploy with persistent, writable storage.
...
- `prisma/migrations` is the active SQLite history; use `pnpm exec prisma migrate dev --name <name>` for schema changes. `prisma/migrations-postgresql` is historical reference only and must not be applied to SQLite.
...
- A test-only `/login` route (`app/(customer)/login`) accepts e-mail/password only when `TEST_LOGIN_ENABLED=true` (password comes from `TEST_LOGIN_PASSWORD`, set only in `.env.local`): it creates a database `Session` directly and redirects admins to `/dashboard`, customers to `/bookings`. Never enable it in production — `checkDeployment` does NOT flag the variable (PRD RISK-021) and the action has no rate limit of its own.
